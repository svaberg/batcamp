...
def _append_report_runtime_section(
    lines: list[str],
    rows: list[dict[str, float | int]],
    *,
    linear_enabled: bool,
) -> None:
    """Append steady-state runtime rows to one markdown report."""
    if linear_enabled:
        lines.extend(
            [
...
def _write_report(
    ...
    *,
    ...
    variable: str,
    tree_coord: str,
    build_times: dict[str, float | None],
    linear_enabled: bool,
    linear_reference_mode: str,
    linear_reference_note: str | None,
    cold_times: dict[str, float | int | None],
) -> None:
    """Write one readable markdown summary with build and query timings."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    octree_tree_s = float(build_times["octree_tree_s"])
    octree_interp_s = float(build_times["octree_interp_s"])
    nearest_build_s = float(build_times["nearest_build_s"])
    linear_delaunay_s = build_times["linear_delaunay_s"]
    linear_interp_s = build_times["linear_interp_s"]
    cold_query_count = int(cold_times["query_count"])
    octree_cold_s = float(cold_times["octree_s"])
    linear_cold_s = cold_times["linear_s"]
    nearest_cold_s = float(cold_times["nearest_s"])
    ...
    _append_report_runtime_section(lines, rows, linear_enabled=linear_enabled)
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _run_case(
    *,
    ...
    max_seconds_per_query: float,
    variable: str,
) -> None:
    """Run one dataset case through the full random-point benchmark."""
    ...
    interp, octree_interp_s = _time_call(OctreeInterpolator, tree, np.asarray(ds[variable], dtype=float))
    ...
    cold_times = {
        "query_count": int(warm_count),
        "octree_s": float(octree_warm_s),
        "linear_s": None if linear_interp is None else float(linear_warm_s),
        "nearest_s": float(nearest_warm_s),
    }
...
def main() -> None:
    ...
    for case_index, case in enumerate(cases):
        _run_case(
            ...
            max_seconds_per_query=max_seconds_per_query,
