...
def _save_accuracy_plot(
    ...
    *,
    ...
    """Save error and overlap curves using SciPy linear as the reference."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    query_counts = np.asarray([int(row["query_count"]) for row in rows], dtype=float)

    oct_abs_mae = np.asarray([float(row["octree_vs_linear_abs_mae"]) for row in rows], dtype=float)
    oct_abs_rmse = np.asarray([float(row["octree_vs_linear_abs_rmse"]) for row in rows], dtype=float)
    near_abs_mae = np.asarray([float(row["nearest_vs_linear_abs_mae"]) for row in rows], dtype=float)
    near_abs_rmse = np.asarray([float(row["nearest_vs_linear_abs_rmse"]) for row in rows], dtype=float)

    oct_log_mae = np.asarray([float(row["octree_vs_linear_log10_mae"]) for row in rows], dtype=float)
    oct_log_rmse = np.asarray([float(row["octree_vs_linear_log10_rmse"]) for row in rows], dtype=float)
    near_log_mae = np.asarray([float(row["nearest_vs_linear_log10_mae"]) for row in rows], dtype=float)
    near_log_rmse = np.asarray([float(row["nearest_vs_linear_log10_rmse"]) for row in rows], dtype=float)

    oct_overlap = np.asarray([int(row["octree_vs_linear_finite_overlap"]) for row in rows], dtype=float)
    oct_positive = np.asarray([int(row["octree_vs_linear_positive_overlap"]) for row in rows], dtype=float)
    near_overlap = np.asarray([int(row["nearest_vs_linear_finite_overlap"]) for row in rows], dtype=float)
    near_positive = np.asarray([int(row["nearest_vs_linear_positive_overlap"]) for row in rows], dtype=float)

    fig, axes = plt.subplots(1, 3, figsize=(17.0, 5.3))

    axes[0].plot(query_counts, oct_abs_mae, "o-", label="octree vs linear MAE")
    axes[0].plot(query_counts, oct_abs_rmse, "o-", label="octree vs linear RMSE")
    axes[0].plot(query_counts, near_abs_mae, "s--", label="nearest vs linear MAE")
    axes[0].plot(query_counts, near_abs_rmse, "s--", label="nearest vs linear RMSE")
    ...
    axes[1].plot(query_counts, oct_log_mae, "o-", label="octree vs linear MAE")
    axes[1].plot(query_counts, oct_log_rmse, "o-", label="octree vs linear RMSE")
    axes[1].plot(query_counts, near_log_mae, "s--", label="nearest vs linear MAE")
    axes[1].plot(query_counts, near_log_rmse, "s--", label="nearest vs linear RMSE")
    ...
    axes[2].plot(query_counts, oct_overlap, "o-", label="octree vs linear finite")
    axes[2].plot(query_counts, oct_positive, "o--", label="octree vs linear positive")
    axes[2].plot(query_counts, near_overlap, "s-", label="nearest vs linear finite")
    axes[2].plot(query_counts, near_positive, "s--", label="nearest vs linear positive")
    ...
    fig.suptitle(f"{dataset_label}: {linear_label} as reference")
...
def _save_parity_scatter(
    ...
    *,
    ...
    """Save one parity scatter figure for the largest query batch."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig, axes = plt.subplots(1, 2, figsize=(12.0, 5.7))
    _parity_axis(
        axes[0],
        reference_vals=linear_vals,
        candidate_vals=octree_vals,
        title=f"Octree at {query_count} random queries",
        ylabel="Octree",
        xlabel=linear_label,
    )
    _parity_axis(
        axes[1],
        reference_vals=linear_vals,
        candidate_vals=nearest_vals,
        title=f"SciPy nearest at {query_count} random queries",
        ylabel="SciPy Nearest",
        xlabel=linear_label,
    )
    fig.suptitle(f"{dataset_label}: parity against {linear_label}")
...
def main() -> None:
    ...
    for case_index, case in enumerate(cases):
        ...
        (tree, tree_source), octree_tree_s = _time_call(
        ...
        rows: list[dict[str, float | int]] = []
        largest_linear_vals = np.array([], dtype=float)
        ...
        for n_query in query_counts:
            ...
            if linear_enabled and linear_label is not None:
                _save_accuracy_plot(
                    rows,
                    case_dir / "accuracy_vs_queries.png",
                    dataset_label=case.label,
