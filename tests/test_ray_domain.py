...
def _build_trace_rpa_tree() -> Octree:
    """Return one denser spherical tree for first-step xyz tracing coverage."""
    points, corners = build_spherical_hex_mesh(
        nr=2,
        npolar=4,
        nazimuth=8,
...
def test_trace_seed_step_with_explicit_interior_seed_returns_one_cartesian_segment() -> None:
    ...
    segments = tracer.trace_seed_step(origin, direction, seed_xyz=seed_xyz)
...
def test_trace_seed_step_splits_cartesian_boundary_seed_into_two_segments() -> None:
    ...
    segments = tracer.trace_seed_step(origin, direction)
...
def test_trace_seed_step_rpa_segments_lookup_back_to_their_cells() -> None:
    """The first traced spherical seed-step segments should be self-consistent in xyz."""
